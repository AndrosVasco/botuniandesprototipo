...
export async function sendChatMessage(sessionId: string, message: string, mode: ChatMode, language: Language): Promise<ChatResponse> {
  const response = await fetch(`${API_URL}/api/chat`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId, message, mode, language }) });
